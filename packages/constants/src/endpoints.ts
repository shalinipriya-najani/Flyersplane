...
const normalizeLoopbackUrl = (value: string): string => {
  ...
  if (!browserLocation?.hostname) return value;

  try {
    const url = new URL(value);
    const currentHostname = browserLocation.hostname;

    if (LOOPBACK_HOSTS.has(url.hostname) && LOOPBACK_HOSTS.has(currentHostname)) {
...
export const API_BASE_URL = normalizeLoopbackUrl(process.env.VITE_API_BASE_URL || "");
...
export const ADMIN_BASE_URL = normalizeLoopbackUrl(process.env.VITE_ADMIN_BASE_URL || "");
export const ADMIN_BASE_PATH = process.env.VITE_ADMIN_BASE_PATH || "";
export const GOD_MODE_URL = encodeURI(`${ADMIN_BASE_URL}${ADMIN_BASE_PATH}`);
// Publish App Base Url
export const SPACE_BASE_URL = normalizeLoopbackUrl(process.env.VITE_SPACE_BASE_URL || "");
...
// Live App Base Url
export const LIVE_BASE_URL = normalizeLoopbackUrl(process.env.VITE_LIVE_BASE_URL || "");
export const LIVE_BASE_PATH = process.env.VITE_LIVE_BASE_PATH || "";
export const LIVE_URL = encodeURI(`${LIVE_BASE_URL}${LIVE_BASE_PATH}`);
// Web App Base Url
export const WEB_BASE_URL = normalizeLoopbackUrl(process.env.VITE_WEB_BASE_URL || "");
